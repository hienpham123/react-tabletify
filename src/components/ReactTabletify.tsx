...
export function ReactTabletify<T extends Record<string, any>>({
  ...
  const handleSelectAll = React.useCallback((checked: boolean) => {
    if (selectionMode === 'multiple') {
      const currentPageItems = table.paged;
      setSelectedItems((prev) => {
        const newSet = new Set(prev);
        if (checked) {
          currentPageItems.forEach((item, i) => {
            newSet.add(getItemKey(item, i));
          });
        } else {
          currentPageItems.forEach((item, i) => {
            newSet.delete(getItemKey(item, i));
          });
        }
  ...
  }, [selectionMode, table.paged, getItemKey, onSelectionChanged, data]);
  ...
  const isAllSelected = React.useMemo(() => {
    ...
    return currentPageItems.every((item, i) => selectedItems.has(getItemKey(item, i)));
  }, [selectionMode, table.paged, selectedItems, getItemKey]);
  ...
  const isIndeterminate = React.useMemo(() => {
    ...
    const currentPageItems = table.paged;
    if (currentPageItems.length === 0) return false;
    const selectedCount = currentPageItems.filter((item, i) => selectedItems.has(getItemKey(item, i))).length;
  ...
  return (
    <div className={`th-table ${className || ''}`} style={styles}>
      <table>
        <thead>
          <tr>
            {selectionMode !== 'none' && (
              ...
              <th key={String(col.key)}>
                ...
                {calloutKey === col.key && (
                  <HeaderCallout
                    ...
                    onFilter={() => handleOpenFilter(col.key)}
                    ...
                    rows.map((row, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {columns.map((col) => (
                            <td key={String(col.key)}>{renderCell(row, col, finalIndex)}</td>
                          ))}
                        </tr>
                      );
                    })}
            ...
            table.paged.map((row, i) => {
              ...
              return (
                ...
                >
                  ...
                  {columns.map((col) => (
                    <td key={String(col.key)}>{renderCell(row, col, i)}</td>
                  ))}
                </tr>
              );
            })
